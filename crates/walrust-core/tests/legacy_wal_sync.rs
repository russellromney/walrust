...
use walrust_core::legacy_wal_sync::{
    snapshot_database_to_storage, snapshot_database_to_storage_with_post_encode, sync_wal_to_cache,
    sync_wal_to_storage, sync_watched_db_once_to_cache, take_snapshot_to_storage, SyncInput,
    WatchedDbState,
};
            ...
            anyhow::ensure!(
                output.status.success()
                    && String::from_utf8_lossy(&output.stdout)
                        .contains("external checkpoint busy=1"),
                "cross-process TRUNCATE must be blocked during storage put: {output:?}"
            );
        }
        self.objects
            .lock()
...
async fn shadow_blocker_survives_two_production_snapshots_and_ephemeral_writers() -> Result<()> {
    ...
    let mut blocker = Some(ShadowWal::open_checkpoint_blocker(&db_path)?);
    let storage = MemoryStorage {
        checkpoint_probe_db: Some(db_path.clone()),
        ..MemoryStorage::default()
    };
    for _ in 0..2 {
        snapshot_database_to_storage_with_post_encode(
            &storage,
            "backups",
            "shadow-snapshot",
            &db_path,
            || {
                let old_blocker = blocker.take().unwrap();
                old_blocker.execute_batch("ROLLBACK")?;
                drop(old_blocker);
                // POSIX-safe ordering: all snapshot handles are closed before
                // the replacement blocker opens, so storage.put's external
                // process sees the replacement read lock.
                blocker = Some(ShadowWal::open_checkpoint_blocker(&db_path)?);
                Ok(())
            },
        )
        .await?;
    ...
    assert_eq!(busy, 1);
    blocker.unwrap().execute_batch("ROLLBACK")?;
    Ok(())
}
