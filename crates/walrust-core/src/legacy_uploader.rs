...
use crate::legacy_manifest::build_ltx_key;
use anyhow::{anyhow, Context, Result};
use hadb_io::retry::{classify_error, ErrorKind, RetryPolicy};
...
impl UploadTaskContext {
    ...
    async fn upload_txid(&self, txid: u64) -> Result<UploadResult> {
        ...
        if let Err(e) = crate::legacy_ltx::verify_ltx(Cursor::new(&data))
...
mod tests {
    ...
    fn test_ltx(txid: u64) -> Vec<u8> {
        use crate::legacy_ltx::Checksum;
        use crate::legacy_ltx::{chain_checksum, encode_wal_changes};
