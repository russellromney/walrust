...
//!
//! By default, walrust restores from native HADBP snapshot files stored in S3.
//! When a `SnapshotSource` is provided, walrust calls `materialize()` instead
//! of downloading a HADBP snapshot. This enables integration with external
//! page-group storage (e.g., turbolite) where page groups serve as the snapshot.
//!
//! Usage:
//! ```ignore
//! // Standard restore (HADBP snapshots from S3):
...
/// of downloading a HADBP snapshot. After materialization, walrust applies
/// any HADBP WAL increments with txid > the returned checkpoint version.
///
...
pub trait SnapshotSource: Send + Sync {
    ...
    ///
    /// This replaces the HADBP snapshot download step in `restore()`.
