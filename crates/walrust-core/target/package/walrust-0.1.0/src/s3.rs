...
mod tests {
    ...
    // Integration tests - requires S3 credentials (AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY)
    // Bucket: WALRUST_TEST_BUCKET or HA_BUCKET
    // Endpoint: AWS_ENDPOINT_URL_S3 or AWS_ENDPOINT_URL

    fn get_test_bucket() -> Option<String> {
        std::env::var("WALRUST_TEST_BUCKET")
            .or_else(|_| std::env::var("HA_BUCKET"))
            .ok()
    }
    ...
    async fn test_integration_upload_download_bytes() {
        let bucket = match get_test_bucket() {
            Some(b) => b,
            None => return, // No credentials — skip gracefully
        };
