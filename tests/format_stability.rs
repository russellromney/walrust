//! Format-stability proof for the published native HADBP library protocol.
//!
//! The frozen owned-v0.7.0 bucket must continue restoring latest and PITR
//! row-exact through the public Replicator restore path.
...
// reach (the owned fixture's L2 predates the first snapshot). Their bytes are
// still frozen 0.7.0 output, so decode every HADBP payload — merged `levels/`
// objects and every `.hadbp` snapshot/delta — with the current decoder.
// `decode_sqlite_changeset` verifies the embedded content checksum, so a
// format break or a single flipped byte fails loudly.
...
/// A bucket written by the published walrust 0.7.0 owned-mode API restores
/// row-exact through the current native HADBP library path: latest + PITR +
/// integrity.
#[tokio::test]
